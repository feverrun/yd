...
let zqlookStartbody= $.isNode() ? (process.env.zqlookStartbody1 ? process.env.zqlookStartbody1 : "") : ($.getdata('zqlookStartbody') ? $.getdata('zqlookStartbody') : "")
...
else if (process.env.zqlookStartbody1 && process.env.zqlookStartbody1.indexOf('&') > -1) {
    zqlookStartbodyArr = process.env.zqlookStartbody1.split('&');
    console.log(`您选择的是用"&"隔开\n`)
}
else {
    zqlookStartbodys = [process.env.zqlookStartbody1]
};
