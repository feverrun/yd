...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!$.isNode()) {
            khhdArr.push($.getdata('khhd'))
            let yyxnycount = ($.getval('yyxnycount') || '1');
            for (let i = 2; i <= yyxnycount; i++) {
                khhdArr.push($.getdata(`yyxnyhd${i}`))
            }
            console.log(`------------- 共${khhdArr.length}个账号-------------\n`)
            for (let i = 0; i < khhdArr.length; i++) {
                if (khhdArr[i]) {
                    khhd = khhdArr[i];
                    $.index = i + 1;
            ...
            if (process.env.KH_HD && process.env.KH_HD.indexOf('@') > -1) {
                khhdArr = process.env.KH_HD.split('@');
                console.log(`您选择的是用"@"隔开\n`)
            } else {
                khhds = [process.env.KH_HD]
            }

            Object.keys(khhds).forEach((item) => {
                if (khhds[item]) {
                    khhdArr.push(khhds[item])
                }
            })
            console.log(`共${khhdArr.length}个cookie`)
            for (let k = 0; k < khhdArr.length; k++) {
                $.message = ""
                khhd = khhdArr[k]
                $.index = k + 1;
                console.log(`\n开始【爷爷的小农院${$.index}】`)
...
function yyxnyck() {
    if ($request.url.indexOf("sync") > -1) {
        const khhd = JSON.stringify($request.headers)
        if (khhd) $.setdata(khhd, `yyxnyhd${status}`)

        $.log(khhd)
        $.msg($.name, "", '酷划' + `${status}` + '数据获取成功！')
    }
}
