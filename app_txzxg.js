...
let txzxg_openid = $.getdata('txzxg_openid')
...
//||process.env.txzxg_openid!==''
//console.log(process.env.txzxg_openid)
        if(!process.env.txzxg_openid){
            console.log(`\n【${$.name}】：未提供变量 txzxg_openid`);
            return;
        }else if(process.env.txzxg_token=='undefined'){
            console.log(`\n【${$.name}】：未提供变量 txzxg_token`);
            return;
        }else if(process.env.txzxg_fskey=='undefined'){
            console.log(`\n【${$.name}】：未提供变量 txzxg_openid`);
            return;
        }

        //变量txzxg_openid
        if (process.env.txzxg_openid && process.env.txzxg_openid.indexOf('@') > -1) {
            txzxg_openid = process.env.txzxg_openid.split('@');
        } else if (process.env.txzxg_openid && process.env.txzxg_openid.indexOf('\n') > -1) {
            txzxg_openid = process.env.txzxg_openid.split('\n');
        } else if(process.env.txzxg_openid && process.env.txzxg_openid.indexOf('#') > -1){
            txzxg_openid = process.env.txzxg_openid.split('#');
        }else{
            txzxg_openid = process.env.txzxg_openid.split();
        };

        Object.keys(txzxg_openid).forEach((item) => {
            if (txzxg_openid[item]) {
                app_txzxg_openid.push(txzxg_openid[item]);
            };
        });

        //变量txzxg_token
        if (process.env.txzxg_token && process.env.txzxg_token.indexOf('@') > -1) {
            txzxg_token = process.env.txzxg_token.split('@');
        } else if (process.env.txzxg_token && process.env.txzxg_token.indexOf('\n') > -1) {
            txzxg_token = process.env.txzxg_token.split('\n');
        } else if(process.env.txzxg_token && process.env.txzxg_token.indexOf('#') > -1){
            txzxg_token = process.env.txzxg_token.split('#');
        }else{
            txzxg_token = process.env.txzxg_token.split();
        };

        Object.keys(txzxg_token).forEach((item) => {
            if (txzxg_token[item]) {
                app_txzxg_token.push(txzxg_token[item]);
            };
        });

        //变量txzxg_fskey
        if (process.env.txzxg_fskey && process.env.txzxg_fskey.indexOf('@') > -1) {
            txzxg_fskey = process.env.txzxg_fskey.split('@');
        } else if (process.env.txzxg_fskey && process.env.txzxg_fskey.indexOf('\n') > -1) {
            txzxg_fskey = process.env.txzxg_fskey.split('\n');
        } else if(process.env.txzxg_fskey && process.env.txzxg_fskey.indexOf('#') > -1){
            txzxg_fskey = process.env.txzxg_fskey.split('#');
        }else{
            txzxg_fskey = process.env.txzxg_fskey.split();
        };

        Object.keys(txzxg_fskey).forEach((item) => {
            if (txzxg_fskey[item]) {
                app_txzxg_fskey.push(txzxg_fskey[item]);
            };
        });

        //变量txzxg_dev
        if(!process.env.txzxg_dev||process.env.txzxg_dev==''){
            console.log(`\n【${$.name}】：未提供变量 txzxg_dev ,将默认分配`);
            txzxg_dev='Redmi'
        }else{
            if (process.env.txzxg_dev && process.env.txzxg_dev.indexOf('@') > -1) {
                txzxg_dev = process.env.txzxg_dev.split('@');
            } else if (process.env.txzxg_dev && process.env.txzxg_dev.indexOf('\n') > -1) {
                txzxg_dev = process.env.txzxg_dev.split('\n');
        ...
        if(!process.env.txzxg_mid||process.env.txzxg_mid==''){
            ...
            if (process.env.txzxg_mid && process.env.txzxg_mid.indexOf('@') > -1) {
                txzxg_mid = process.env.txzxg_mid.split('@');
            } else if (process.env.txzxg_mid && process.env.txzxg_mid.indexOf('\n') > -1) {
                txzxg_mid = process.env.txzxg_mid.split('\n');
            } else if(process.env.txzxg_mid && process.env.txzxg_mid.indexOf('#') > -1){
                txzxg_mid = process.env.txzxg_mid.split('#');
            }else{
                txzxg_mid = process.env.txzxg_mid.split();
            };

            Object.keys(txzxg_mid).forEach((item) => {
                if (txzxg_mid[item]) {
                    app_txzxg_mid.push(txzxg_mid[item]);
                };
            });
        }
        ...
        if(!process.env.txzxg_md5mid||process.env.txzxg_md5mid==''){
            ...
            } else if (process.env.txzxg_md5mid && process.env.txzxg_md5mid.indexOf('\n') > -1) {
                txzxg_md5mid = process.env.txzxg_md5mid.split('\n');
            } else if(process.env.txzxg_md5mid && process.env.txzxg_md5mid.indexOf('#') > -1){
                txzxg_md5mid = process.env.txzxg_md5mid.split('#');
            }else{
                txzxg_md5mid = process.env.txzxg_md5mid.split();
            };

            Object.keys(txzxg_md5mid).forEach((item) => {
                if (txzxg_md5mid[item]) {
                    app_txzxg_md5mid.push(txzxg_md5mid[item]);
                };
            });
        }
        ...
        if(!process.env.txzxg_devid||process.env.txzxg_devid==''){
            ...
            } else if (process.env.txzxg_devid && process.env.txzxg_devid.indexOf('\n') > -1) {
                txzxg_devid = process.env.txzxg_devid.split('\n');
            } else if(process.env.txzxg_devid && process.env.txzxg_devid.indexOf('#') > -1){
                txzxg_devid = process.env.txzxg_devid.split('#');
            }else{
                txzxg_devid = process.env.txzxg_devid.split();
            };

            Object.keys(txzxg_devid).forEach((item) => {
                if (txzxg_devid[item]) {
                    app_txzxg_devid.push(txzxg_devid[item]);
                };
            });
        }
        ...
        if (typeof $request !== "undefined") {
            ...
            if(!$.getdata('txzxg_dev')){
                console.log(`\n【${$.name}】：未提供变量 txzxg_openid`);
                return;
            }else if($.getdata('txzxg_token')){
                console.log(`\n【${$.name}】：未提供变量 txzxg_token`);
                return;
            }else if($.getdata('txzxg_fskey')){
                console.log(`\n【${$.name}】：未提供变量 txzxg_openid`);
                return;
            }

            app_txzxg_openid.push($.getdata('txzxg_openid'))
            app_txzxg_token.push($.getdata('txzxg_token'))
            app_txzxg_fskey.push($.getdata('txzxg_fskey'))

            //变量txzxg_devapp_txzxg_dev.push($.getdata('txzxg_dev'))
            if(!$.getdata('txzxg_dev')|| $.getdata('txzxg_dev')==''){
                console.log(`\n【${$.name}】：未提供变量 txzxg_dev ,将默认分配`);
                //txzxg_dev='Redmi'
            }else{
                app_txzxg_dev.push($.getdata('txzxg_dev'));
    ...
    for (i = 0; i < app_txzxg_openid.length; i++) {
        ...
        if(!process.env.txzxg_dev|| !$.getdata(`txzxg_dev`)){
            txzxg_dev='Redmi'
        }else{
            txzxg_dev=app_txzxg_dev[i]
        }
        ...
        //查看今日大盘行情
        if(dp_status==1){
            await txzxg_task_ticket(dp_id,dp_tid)
            if(task_ticket_state==1){
                await txzxg_gettask(dp_title)
            }
        }
        //浏览行情-港股
        if(gg_status==1){
            await txzxg_task_ticket(gg_id,gg_tid)
            if(task_ticket_state==1){
                await txzxg_gettask(gg_title)
            }

        }
        //阅读一篇资讯
        if(dzx_status==1){
            await txzxg_task_ticket(dzx_id,dzx_tid)
            if(task_ticket_state==1){
                await txzxg_gettask(dzx_title)
            }

        }
        //查看热搜并添加自选
        if(rzx_status==1){
            await txzxg_task_ticket(rzx_id,rzx_tid)
            if(task_ticket_state==1){
                await txzxg_gettask(rzx_title)
            }

        }
        //添加精选策略股票到自选
        if(jzx_status==1){
            await txzxg_task_ticket(jzx_id,jzx_tid)
            if(task_ticket_state==1){
        ...
        //添加龙虎榜股票到自选
        if(lzx_status==1){
            await txzxg_task_ticket(lzx_id,lzx_tid)
            if(task_ticket_state==1){
                await txzxg_gettask(lzx_title)
            }

        }
        //关注一个社区用户
        if(gz_status==1){
            await txzxg_task_ticket(gz_id,gz_tid)
            if(task_ticket_state==1){
                await txzxg_gettask(gz_title)
            }

        }
        //浏览选股题材热点
        if(krd_status==1){
            await txzxg_task_ticket(krd_id,krd_tid)
            if(task_ticket_state==1){
                await txzxg_gettask(krd_title)
            }

        }
        //分享个股行情
        if(fhq_status==1){
            //await txzxg_task_ticket(fhq_id,fhq_tid)
            //if(task_ticket_state==1){
            await txzxg_getapp_ggCode()
            //await txzxg_gettask(fhq_title)
            //}

        }
        //分享一篇资讯
        if(fzx_status==1){
            //await txzxg_task_ticket(fzx_id,fzx_tid)
            //if(task_ticket_state==1){
            await txzxg_getapp_zxCode()
            //await txzxg_gettask(fzx_title)
            //}
...
function get_appdata() {
    if ($request.url.indexOf("readerInfo") > -1) {
        const txzxg_headers = JSON.stringify($request.headers)
        if(txzxg_headers){
            $.setdata(txzxg_headers,`txzxg_headers${status}`)
            console.log(`\n【${$.name}---获取数据】: 成功获取到协议头`)
            subTitle+=`\n【${$.name}---获取数据】: 成功获取到协议头`
        }else{
            console.log(`\n【${$.name}---获取数据】: 没有获取到协议头`)
...
//获取票据
function txzxg_task_ticket(id,tid) {
    return new Promise((resolve, reject) => {
        $.get({
            url : txzxg_task_ticket_url,
            headers : txzxg_headers,
            //body : ``,
        }, async(error, response, data) => {
            //console.log(data)
...
//获取分享个股行情分享code
function txzxg_getapp_ggCode() {
    return new Promise((resolve, reject) => {
        $.get({
            url : `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?action=query_share_code&channel=1&share_type=task_51_1111&uin=${txzxg_openid}&check=11&appid=wxcbc3ab3807acb685&openid=${txzxg_openid}&fskey=${txzxg_fskey}&access_token=${txzxg_token}&g_openid=${txzxg_openid}&uin=${txzxg_openid}&_appName=android&_dev=${txzxg_devid}&_devId=${txzxg_devid}&_mid=${txzxg_devid}&_md5mid=${txzxg_md5mid}&_appver=9.4.1&_ifChId=3178&_screenW=1080&_screenH=2210&_osVer=10&_uin=${txzxg_openid}&_wxuin=${txzxg_openid}&_net=WIFI&__random_suffix=87512&_buildtype=store&buildtime=2021-09-29+17%3A01%3A24&lang=zh_CN`,
...
//获取分享资讯code
function txzxg_getapp_zxCode() {
    return new Promise((resolve, reject) => {
        $.get({
            url : `https://wzq.tenpay.com/cgi-bin/activity/activity_share.fcgi?action=query_share_code&share_type=news_share&channel=1&zappid=zxg_h5&sign=5f778eabaebc6a61d6415d729f29530c&nonce=5500&timestamp=1635602383726&check=11&appid=wxcbc3ab3807acb685&openid=${txzxg_openid}&fskey=${txzxg_fskey}&access_token=${txzxg_token}&g_openid=${txzxg_openid}&uin=${txzxg_openid}&_appName=android&_dev=SKW-A0&_devId=${txzxg_devid}&_mid=${txzxg_devid}&_md5mid=${txzxg_md5mid}&_appver=9.4.1&_ifChId=3178&_screenW=1080&_screenH=2210&_osVer=10&_uin=${txzxg_openid}&_wxuin=${txzxg_openid}&_net=WIFI&__random_suffix=78034&_buildtype=store&buildtime=2021-09-29+17%3A01%3A24&lang=zh_CN&hunheshiType=shy`,
            headers : {"Host": "wzq.tenpay.com","referer": "http://zixuanguapp.finance.qq.com","accept-encoding": "gzip","user-agent":"okhttp/3.12.13"},
            //body : ``,
        }, async(error, response, data) => {
            //console.log(data)
